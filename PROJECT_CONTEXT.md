...
`organization_offices` / `jurisdictions`（多対多）の正規化構造へ再設計。福岡県72判定単位（自治体数は
60市町村、政令指定都市2市を区単位に分割）・法務局2／税務署18／年金事務所11／労基署12／
ハローワーク17を投入。旧`jurisdiction_offices`は削除せず残置。
...
### Phase 6（Sprint42〜44）— クローズドβ launch準備・開始確認
実際の税理士・会計事務所にCompany Workspaceを試用してもらうためのクローズドβ計画を整備した
（Sprint42）。新機能追加は行わず、βテスト対象者・利用開始手順・管理者アカウント発行手順・既知の制約・
重大障害の定義とβ停止条件等を[CLOSED_BETA_LAUNCH_PLAN.md](docs/CLOSED_BETA_LAUNCH_PLAN.md)・
[BETA_TEST_CHECKLIST.md](docs/BETA_TEST_CHECKLIST.md)・
[BETA_FEEDBACK_TEMPLATE.md](docs/BETA_FEEDBACK_TEMPLATE.md)として整備した。続けてSprint43
「Beta Reliability Polish」で、`loading.tsx`（Workspace関連7ルート）・`error.tsx`
（`/admin/workspaces`配下）・owner限定のWorkspace削除UI（`WorkspaceDeleteButton.tsx`、既存の
`ON DELETE CASCADE`を利用）を実装した。Sprint44「Closed Beta Start」でβ開始前の最終確認
（Smoke Test・既知の制約の棚卸し・住民税特別徴収未実装の確認）を行い、
[CLOSED_BETA_DAY1_RUNBOOK.md](docs/CLOSED_BETA_DAY1_RUNBOOK.md)（初日実行手順10ステップ）を新設、
β関連ドキュメント一式をSprint43・44の実装状況に同期した。「軽微な準備後に開始可能」と判定し、
β自体は本ドキュメント時点では未実施。

## 現在の状態（2026-07-11時点、Sprint44で同期）
...
- **対応エリア**: 東京都渋谷区（Phase 1）＋ 福岡県全域（60市町村、Resolverの管轄判定単位では72判定単位、Phase 1.5）
...
- **β提供**: Sprint42でクローズドβ launch計画を整備し、Sprint43でβ前軽微修正（loading/error境界・
  Workspace削除UI）、Sprint44でβ開始前の最終確認を行った（判定: 軽微な準備後に開始可能。β自体は
  未実施）。詳細は[CLOSED_BETA_LAUNCH_PLAN.md](docs/CLOSED_BETA_LAUNCH_PLAN.md)・
  [CLOSED_BETA_DAY1_RUNBOOK.md](docs/CLOSED_BETA_DAY1_RUNBOOK.md)
- **未着手・既知の制約**:
  - 東京都・福岡県以外の都道府県は未対応
  ...
  - 住民税特別徴収（毎月納付・普通徴収）に関するProcedureがProcedure Masterに未登録のため、
    年間ロードマップに表示されない（Sprint44調査、Engineの不具合ではなくデータ未登録が原因。
    実装する場合は毎月納付と納期の特例をセットで設計する必要がある）
  - `workspace_members`への割り当て・Workspace（会社）自体の一覧編集にはUIが無く、Supabase
    SQL Editorでの手動操作が必要（会社作成時の自動owner登録、およびowner本人による削除
    （Sprint43実装、`WorkspaceDeleteButton.tsx`）を除く）
...
Sprint40〜41でv1.0 Release Candidate Reviewとβ前軽微修正、Sprint42〜44でクローズドβ launch計画の
整備・信頼性向上（loading/error境界・Workspace削除UI）・開始前最終確認を行った（詳細は本ファイル
「完了済みPhase」Phase5・Phase6、[CLOSED_BETA_LAUNCH_PLAN.md](docs/CLOSED_BETA_LAUNCH_PLAN.md)を参照）。
v0.7（補助金・助成金）・v1.0（福岡県版正式リリース）は未着手。
