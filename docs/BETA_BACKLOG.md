...
5. **【Sprint88で追加】** 実際の外部参加者（税理士／会計事務所スタッフ／社長）をファシリテーターが
   横で観察しながら1操作単位で記録する場合は、[BETA_DAY1_OBSERVATION.md](BETA_DAY1_OBSERVATION.md)に
   記入したのち、Blocker/High判定が付いた記録を本ファイルへ転記する（発見元は`Day1Observation`、
   記録番号を付記。例：`Day1Observation #003`）。分類定義（Blocker/High/Medium/Low）は
   `BETA_PILOT_LOG.md`と同一のものを使用する。`BETA_DAY1_OBSERVATION.md`では実害の分類に加えて
   「Nice to Have」（無くても困らない要望）と「原因分類」（UI／文言／導線／説明不足／データ不足／
   業務知識／速度／その他）も記録するが、これらは本ファイルの優先順位ルール（0-2節）には影響しない
   補助情報として扱う
...
*（注: 福岡県の自治体数は60市町村、Resolverの管轄判定単位は72判定単位。本エントリは発見時点の記載をそのまま保持する）*
