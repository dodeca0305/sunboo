...
prefectures ─┬─ municipalities ─┬─ jurisdiction_offices（旧設計・残置・未参照）
             ...
             │
             └─ anonymous_company_events（法人所在地の市区町村を保持）
...
event_types ─┬─ anonymous_company_events（どのイベントが登録されたか）
...
### `anonymous_company_events`
登録された経営イベント本体。アプリの設計としては**永続的な`companies`という独立エンティティは
存在しない**（一般ユーザーにアカウント機能が無いため）。会社プロフィール（`municipality_id`,
`corporate_type`, `has_employees`）はイベントごとに非正規化して直接保持し、`browser_id`
（クライアント生成UUID、`localStorage`に保存）で「同じブラウザが登録したイベント」を緩やかに束ねる。
...
> **命名についての注記（2026-07-05判明）**: 本来 `company_events` という名前を想定していたが、
> 本番Supabaseプロジェクトには同名の別テーブルが既に存在していた。その`company_events`は
> `company_id`カラムを持ち、`companies`という別テーブル（`auth_user_id`カラム持ち）とRLSで
> 連動する**認証必須のマルチテナント設計**（`own_select`/`own_insert`/`own_update`/`own_delete`/
> `admin_read`ポリシー）で、`migration_event_engine.sql`が定義する匿名`browser_id`ベースの設計とは
> まったく別物だった。`CREATE TABLE IF NOT EXISTS`により無言でスキップされ、このアプリ用の
> テーブルは一度も本番に作成されていなかったことが、`/events`の`permission denied`調査で発覚した。
> アプリケーションコードは`companies`/`auth_user_id`/`company_id`を一切参照しておらず、
> 素性不明のテンプレート的残骸と判断し、**触らずそのまま残置**。衝突を避けるため、このアプリの
> 匿名イベントテーブルは`anonymous_company_events`という名前に変更した。
...
| ロール | 権限 | 対象 |
|---|---|---|
| `anon`（未ログイン一般ユーザー） | `SELECT` 全テーブル、`INSERT`は`anonymous_company_events`のみ | RLSポリシー`public_read`（`USING (true)`）、`anonymous_company_events`は`anon_insert`/`anon_read` |
