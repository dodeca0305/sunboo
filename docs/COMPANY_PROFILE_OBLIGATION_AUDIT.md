...
| 福岡県60市区町村の`municipal_tax`/`prefectural_tax`窓口データ欠落（注: 福岡県の自治体数は60市町村、Resolverの管轄判定単位は72判定単位。本行は執筆時点の記載を保持） | `jurisdictions`データが渋谷区分しか投入されていない。CompanyProfile側は正しく`municipalityCode`を保持・送信している | `docs/BETA_BACKLOG.md` M-02 |
